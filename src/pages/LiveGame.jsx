...
export default function LiveGame() {
  ...
  const loadGameStatus = useCallback(async (roundId) => {
    ...
    try {
      const res = await getGameStatusAPI(roundId);
      if (res.success && res.data) {
        const { called_numbers = [], is_running, last_called_number } = res.data;
  ...
  const setupSocket = useCallback((gameId) => {
    ...
    socket.on("number_called", (data) => {
      ...
      // Ensure we set current number properly
      ...
      // Also set isRunning to true if numbers are being called
      setIsRunning(true);
      addAnnouncement(`📢 Number Called: ${data.number}`, "success");
    });
  ...
  const handleBulkNumbersCall = async () => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        if (newNumbers.length > 0) {
          setCurrent(newNumbers[newNumbers.length - 1]);
        }
  ...
  useEffect(() => {
    // If numbers are being called and game is not running, 
    // it means game is actually running
    if (numbers.length > 0 && !isRunning && selectedRoundId) {
  ...
  }, [numbers.length, isRunning, selectedRoundId]);
          ...
          <div className="control-panel">
            ...
            <div className="control-buttons">
              <button
                className="btn-start"
                onClick={startGame}
                disabled={loading || !selectedRoundId || isRunning || numbers.length > 0}
              >
                {loading ? "..." : "▶ Start"}
              </button>
              <button
