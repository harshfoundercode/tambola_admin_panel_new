...
// //     // ═══════════════════════════════════════════════
// //     // GAME EVENTS - Same as HTML reference
// //     // ═══════════════════════════════════════════════
...
// //       addAnnouncement("❌ Socket connection error", "error");
// //     });
...
// //         // 🔥 Emit pause_game (same as HTML reference)
...
// //       setIsRunning(true);
// //       addAnnouncement("▶️ Game resumed!", "success");
// //       // 🔥 Emit resume_game (same as HTML reference)
...
// //   const uncalledNumbers = availableNumbers.filter(num => !numbers.includes(num));
  
// //   if (uncalledNumbers.length === 0) {
// //     addAnnouncement("❌ No numbers left to call!", "error");
// //     return;
// //   }
  
// //   // Select up to 10 random numbers (you can adjust this number)
// //   const count = Math.min(10, uncalledNumbers.length);
// //   const shuffled = [...uncalledNumbers].sort(() => Math.random() - 0.5);
// //   const randomNumbers = shuffled.slice(0, count);
  
// //   // Add to selected bulk numbers
...
// //   // Update input field preserving order
// //   const selectedArray = Array.from(newSelected);
// //   setBulkNumbersInput(selectedArray.join(', '));
  
// //   addAnnouncement(`🎲 Added ${randomNumbers.length} random numbers to selection`, "success");
// // };
// //   // ── BULK NUMBERS ───────────────────────────────────────────────────────
...
// //   // Keep the order as selected (not sorted)
// //   const selectedArray = Array.from(newSelected);
// //   // We want to keep the order they were added, but since Set preserves insertion order,
// //   // we just need to convert to array without sorting
// //   setBulkNumbersInput(selectedArray.join(', '));
// // };

// //   // 🔥 BULK ANNOUNCEMENT API CALL
...
//         const newNumbers = [...prev, data.number];
//         // Update available numbers immediately
...
//     // Decide how many numbers to select
//     let count;
//     if (selectedBulkNumbers.size === 0) {
//       // Agar koi select nahi hai toh 5-10 random numbers select karein
//       count = Math.min(10, uncalledNumbers.length);
//     } else {
//       // Agar already select hain toh 3-5 aur random numbers add karein
//       count = Math.min(5, uncalledNumbers.length);
//     }
    
//     // Random numbers select karein
//     const shuffled = [...uncalledNumbers].sort(() => Math.random() - 0.5);
//     const randomNumbers = shuffled.slice(0, count);
    
//     // Selected numbers mein add karein
//     const newSelected = new Set(selectedBulkNumbers);
//     randomNumbers.forEach(num => newSelected.add(num));
//     setSelectedBulkNumbers(newSelected);
    
//     // Input field update karein - order preserve karte hue
//     const selectedArray = Array.from(newSelected);
//     setBulkNumbersInput(selectedArray.join(', '));
...
//     } else {
//       addAnnouncement(`🎲 Added ${randomNumbers.length} more random numbers: ${randomNumbers.join(', ')}`, "success");
//     }
//   };
...
//   const handleDeclareWinnerByNumber = async () => {
//   // ✅ Check if round ID exists first
//   if (!selectedRoundId) {
//     addAnnouncement("❌ No active round found. Please start a game first.", "error");
//     return;
//   }

//   const num = parseInt(winnerNumber);
//   if (isNaN(num) || num < 1 || num > TOTAL_NUMBERS) {
//     addAnnouncement("Please enter a valid number to declare as winner", "error");
//     return;
//   }

//   if (!window.confirm(`Declare ALL players who completed number ${num} as winners?`)) {
//     return;
//   }

//   setLoading(true);
//   try {
//     console.log("📤 Declaring winner by number:", {
//       roundId: selectedRoundId,
//       number: num,
//       winType: winnerType
//     });

//     const res = await forceWinnerByNumberAPI(selectedRoundId, { 
//       number: num, 
//       win_type: winnerType 
//     });
...
//   if (!winnerTicketId?.trim()) {
//     addAnnouncement("Please enter a valid Ticket ID", "error");
//     return;
//   }

//   if (!window.confirm(`Declare ticket ${winnerTicketId} as winner for ${winnerType}?`)) {
//     return;
//   }

//   setLoading(true);
//   try {
//     console.log("📤 Declaring winner with:", {
//       roundId: selectedRoundId,
...
export default function LiveGame() {
  ...
  // ✅ Winner by Ticket - Without winner type/category
  const handleDeclareWinnerByTicket = async () => {
    ...
    setLoading(true);
    try {
      console.log("📤 Declaring winner with NEW API:", {
        gameId: selectedGameId,
        ticketNumber: ticketNumber
      });

      // ✅ Using NEW API without winner type
  ...
  return (
    <div className="live-game-container">
      ...
      {/* Admin Panel - Only Bulk Numbers and Winner by Ticket */}
      {showAdminPanel && (
        <div className="admin-panel">
          {/* Bulk Numbers Call with Grid */}
          <div className="admin-section">
