...
export default function GameManager({ onGameSelect }) {
  ...
  // ✅ REMOVED: isFullHouseSelected function - ab zaroorat nahi hai
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ REMOVED: Full House validation for Half Sheet and Full Sheet Bonus
    // ✅ Now bonuses can be added independently
    ...
    try {
      ...
      const gamePayload = {
        ...
        prizes: prizes,
        // ✅ ADDED BACK: These are for backward compatibility
        half_sheet_bonus: form.winning_prizes["Half Sheet Bonus"] || 0,
        full_sheet_bonus: form.winning_prizes["Full Sheet Bonus"] || 0,
      };
    ...
    <div className="game-manager">
      ...
      <div className="form-card">
        ...
        <form onSubmit={handleSubmit}>
          ...
          {/* Winning House Checkboxes - MODIFIED */}
          <div className="form-group">
            <label>Winning House Options</label>
            <div className="checkbox-grid">
              {winningHouseOptions.map((option) => {
                // ✅ Bonus options are now always enabled
                const isBonus = option === "Half Sheet Bonus" || option === "Full Sheet Bonus";
                    ...
                    {form.winning_house.includes(option) && (
                      <div className="prize-inputs">
                        <input
                          ...
                          onChange={(e) => handlePrizeAmountChange(option, e.target.value)}
                          className="prize-input"
                          required={isBonus} // Bonus options require prize amount
                        />
                      </div>
                    )}
                  </div>
            ...
            {/* ✅ Updated hint */}
            <small style={{ color: '#666', fontSize: '12px', display: 'block', marginTop: '8px' }}>
              💡 Select any winning house options. Half Sheet and Full Sheet Bonus can be added independently.
            </small>
          </div>
