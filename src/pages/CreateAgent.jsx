...
function validate(form, isEdit) {
  ...
  if (!isEdit) {
    ...
    // For edit: only validate if password is changed (not empty and different from original)
    // We'll track if password is changed in the component state
    if (form.password && form.password.trim() !== '') {
...
export default function CreateAgent() {
  ...
  // Store the original password from editData
  const originalPassword = editData?.password || '';
  ...
  const [form, setForm] = useState({
    ...
    email:              editData?.email             || '',
    facebook_id:        editData?.facebook_id       || '',
    password:           isEdit ? (editData?.password || '') : '', // Show actual password
    confirmPassword:    '',
  });
  const [errors, setErrors] = useState({});
  const [loading, setLoading] = useState(false);
  ...
  const handleChange = (e) => {
    ...
    // Password field handling - track if changed
    if (name === 'password' && isEdit) {
      // Check if password is different from original
  ...
  const handleSubmit = async (e) => {
    ...
    console.log('Current Password:', form.password);
    console.log('Is Password Changed:', isPasswordChanged);
    console.log('Form Data:', form);
    console.log('Edit Data:', editData);
    ...
    try {
      ...
      if (isEdit) {
        ...
        if (isPasswordChanged && form.password && form.password.trim() !== '') {
          payload.password = form.password;
          console.log('🔑 Password changed! New password will be updated:', payload.password);
        } else {
          console.log('🔑 Password not changed - keeping existing password');
        }
        
        console.log('📤 UPDATE Payload:', payload);
        console.log('Password in payload:', payload.password || 'NOT INCLUDED (keeping existing)');
        ...
        <form className="cag-form" onSubmit={handleSubmit} noValidate>
          ...
          <div className="cag-field">
            <label>
              Password 
              {!isEdit && <span>*</span>}
              {isEdit && <span style={{ fontSize: '0.7rem', color: '#888', marginLeft: '5px' }}>(leave blank to keep current)</span>}
            </label>
            <div className="cag-input-wrap">
              <span className="cag-icon"><FaLock size={14} /></span>
              <input
                ...
                value={form.password}
                onChange={handleChange}
                placeholder={isEdit ? "Enter new password (optional)" : "Min. 6 characters with uppercase, lowercase & number"}
                autoComplete="off"
                className={errors.password ? 'input-error' : ''}
              />
              <button
          ...
          <div className="cag-field">
            <label>
              Confirm Password
              {!isEdit && <span>*</span>}
              {isEdit && <span style={{ fontSize: '0.7rem', color: '#888', marginLeft: '5px' }}>(required if changing password)</span>}
            </label>
            <div className="cag-input-wrap">
              <span className="cag-icon"><FaLock size={14} /></span>
              <input
                ...
                value={form.confirmPassword}
                onChange={handleChange}
                placeholder={isEdit ? "Confirm new password (if changing)" : "Re-enter your password"}
                autoComplete="off"
                className={errors.confirmPassword ? 'input-error' : ''}
              />
              <button
